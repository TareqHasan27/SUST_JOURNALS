...
exports.notifyUser = async (req, res) => {
    try{
        ...
        if(!reg_no){
            return res.status(400).json({message: "reg_no must be needed..", status: "failed"});
        }
        const sql = `
        SELECT
         pr.paper_id,
        ...
        return res.status(400).json({message: "Errror occured.", error: error.message, status: "failed"});
    }
};